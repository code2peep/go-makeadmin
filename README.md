...
P6 已冻结为轻量通用后台首版。核心状态见 [docs/P6_FINAL_STATUS.md](docs/P6_FINAL_STATUS.md)。

已冻结能力：

- 默认本地管理员账号：`admin / 123456`。
- 独立开发库：默认使用 `go_makeadmin`，不和任何业务库混用。
- 自研核心表：默认使用 `ma_*` 系统表，不再依赖旧 `la_*` 运行链路。
...
- 不继续扩展复杂模块市场。
- 不把示例模块当成真实业务功能。
- 不迁移 zyai、PTLM 或其他具体业务。
- 不默认做生产部署。
- 默认本地密码只用于开发环境，生产环境必须自行修改。
...
- 不在当前底座里继续扩展复杂模块市场。
- 不把 zyai、PTLM 或其他业务迁移混入基础框架。

## 文档

推荐先看：

- [本地开发启动链路](docs/LOCAL_DEV.md)
- [P6 最终状态：轻量通用后台首版](docs/P6_FINAL_STATUS.md)
- [P1 最终状态](docs/P1_FINAL_STATUS.md)
- [P2 最终状态](docs/P2_FINAL_STATUS.md)
- [P3 最终状态](docs/P3_FINAL_STATUS.md)
- [P4 最终状态](docs/P4_FINAL_STATUS.md)
- [P5 最终状态](docs/P5_FINAL_STATUS.md)

更细的阶段文档保留在 `docs/` 目录，用于追溯迁移、生成器、模块工具和验收过程。
