...
const modules = {
    ...
    ...import.meta.glob('/src/views/organization/**/*.vue'),
    ...import.meta.glob('/src/views/material/**/*.vue'),
    ...import.meta.glob('/src/views/setting/dict/**/*.vue'),
...
export function createRouteRecord(route: any, firstRoute: boolean): RouteRecordRaw {
    ...
    switch (route.menuType) {
        case MenuEnum.CATALOGUE:
            routeRecord.component = firstRoute ? LAYOUT : markRaw(RouterView)
            if (!route.children) {
