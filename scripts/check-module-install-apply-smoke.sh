...
if [ "${MAKEADMIN_ALLOW_MODULE_INSTALL_SMOKE_WRITE:-}" != "1" ]; then
    echo "FAIL: module install apply smoke requires MAKEADMIN_ALLOW_MODULE_INSTALL_SMOKE_WRITE=1; no database access was attempted."
    exit 1
fi
...
GOCACHE="$GOCACHE" \
go test ./generator/service/gen -run TestModuleManifestInstallApplyLocalSmoke -count=1

echo "OK: module install apply smoke completed."
