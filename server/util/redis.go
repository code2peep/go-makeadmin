...
func (ru redisUtil) client() *redis.Client {
	return core.GetRedis()
}
...
func (ru redisUtil) Info(sections ...string) (res map[string]string) {
	infoStr, err := ru.client().Info(context.Background(), sections...).Result()
...
func (ru redisUtil) DBSize() int64 {
	size, err := ru.client().DBSize(context.Background()).Result()
	if err != nil {
...
func (ru redisUtil) Set(key string, value interface{}, timeSec int) bool {
	err := ru.client().Set(context.Background(),
...
func (ru redisUtil) Get(key string) string {
	res, err := ru.client().Get(context.Background(), config.Config.RedisPrefix+key).Result()
	if err != nil {
...
func (ru redisUtil) SSet(key string, values ...interface{}) bool {
	err := ru.client().SAdd(context.Background(), config.Config.RedisPrefix+key, values...).Err()
	if err != nil {
...
func (ru redisUtil) SGet(key string) []string {
	res, err := ru.client().SMembers(context.Background(), config.Config.RedisPrefix+key).Result()
	if err != nil {
...
func (ru redisUtil) HMSet(key string, mapping map[string]string, timeSec int) bool {
	err := ru.client().HSet(context.Background(), config.Config.RedisPrefix+key, mapping).Err()
	if err != nil {
...
func (ru redisUtil) HGet(key string, field string) string {
	res, err := ru.client().HGet(context.Background(), config.Config.RedisPrefix+key, field).Result()
	if err != nil {
...
func (ru redisUtil) HExists(key string, field string) bool {
	res, err := ru.client().HExists(context.Background(), config.Config.RedisPrefix+key, field).Result()
	if err != nil {
...
func (ru redisUtil) HDel(key string, fields ...string) bool {
	err := ru.client().HDel(context.Background(), config.Config.RedisPrefix+key, fields...).Err()
	if err != nil {
...
func (ru redisUtil) Exists(keys ...string) int64 {
	...
	cnt, err := ru.client().Exists(context.Background(), fullKeys...).Result()
	if err != nil {
...
func (ru redisUtil) Expire(key string, timeSec int) bool {
	err := ru.client().Expire(context.Background(), config.Config.RedisPrefix+key, time.Duration(timeSec)*time.Second).Err()
	if err != nil {
...
func (ru redisUtil) TTL(key string) int {
	td, err := ru.client().TTL(context.Background(), config.Config.RedisPrefix+key).Result()
	if err != nil {
...
func (ru redisUtil) Del(keys ...string) bool {
	fullKeys := ru.toFullKeys(keys)
	err := ru.client().Del(context.Background(), fullKeys...).Err()
	if err != nil {
