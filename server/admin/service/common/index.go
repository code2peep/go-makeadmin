...
func (iSrv indexService) Console() (res map[string]interface{}, e error) {
	...
	return map[string]interface{}{
		...
		"milestones": []map[string]string{
			...
			{"name": "P5 多模块验收", "status": "已冻结", "summary": "Demo Article、Demo Notice、registry smoke 和 no-db 验收完成。"},
			{"name": "P6 通用框架交付", "status": "进行中", "summary": "回到基础后台框架，优先保证可见后台、生成器和 AI 业务扩展约定。"},
		},
		"validation": []map[string]string{
			{"name": "无库验证", "status": "通过", "scope": "runtime residue、Go test、type-check、build、npm audit"},
			{"name": "AI 生成底座", "status": "通过", "scope": "manifest、脚手架、codegen、CRUD 模板、写入门禁"},
			{"name": "开发工具", "status": "通过", "scope": "代码生成器、模块中心、manifest 预览"},
			{"name": "核心页面入口", "status": "通过", "scope": "菜单、角色、管理员、部门、网站信息、缓存、日志"},
			{"name": "通用框架交付", "status": "通过", "scope": "后台首页、核心页面入口、AI 业务生成底座"},
